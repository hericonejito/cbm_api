...
def get_data(dataset_name, preprocess=None, dataset_root=None):
    if dataset_name == "cifar100_train":
        ...
        try:
            data = datasets.Places365(root=os.path.expanduser("~/.cache"), split='train-standard', small=True,
                                      download=True,
                                      transform=preprocess)
        except(RuntimeError):
            data = datasets.Places365(root=os.path.expanduser("~/.cache"), split='train-standard', small=True,
        ...
        if dataset_root is None:
            raise ValueError("Please provide a directory for 'imagenet_broden' dataset.")
        data = torch.utils.data.ConcatDataset(
            [datasets.ImageFolder(os.path.join(dataset_root, "imagenet_val"), preprocess),
             datasets.ImageFolder(os.path.join(dataset_root, "broden"), preprocess)])
    else:
        if dataset_root is None:
            raise ValueError("Please provide a directory for the dataset.")
        print(dataset_root)
        data = datasets.ImageFolder(dataset_root, preprocess)

    return data


def get_targets_only(dataset_name, dataset_root=None):
    pil_data = get_data(dataset_name, dataset_root=dataset_root)
